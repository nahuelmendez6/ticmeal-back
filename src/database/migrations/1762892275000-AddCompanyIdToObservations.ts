...
export class AddCompanyIdToObservations1762892275000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX \`IDX_observations_companyId\` ON \`observations\` (\`companyId\`)`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Eliminar índice
    await queryRunner.query(
      `DROP INDEX \`IDX_observations_companyId\` ON \`observations\``,
    );
